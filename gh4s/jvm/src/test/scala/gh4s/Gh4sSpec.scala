...
abstract class Gh4sSpec extends FunSpec with Matchers with OptionValues with TryValues with EitherValues {
  ...
  implicit protected val scheduler: Scheduler = Scheduler.Implicits.global
  implicit protected val backend              = OkHttpMonixBackend()

  protected val client: GithubClient[Task] = GithubClientBuilder.apiToken(githubToken).build[Task]
